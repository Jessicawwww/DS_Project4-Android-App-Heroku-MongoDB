...
public class CocktailServlet extends HttpServlet {
    ...
    DashboardModel hm = null;
    // Initiate this servlet by instantiating the model that it will use.
    @Override
    public void init() {
        cm = new CocktailModel();
        hm = new DashboardModel();
    }
...
//                throw new RuntimeException(e);
            ...
            try (MongoClient mongoClient = MongoClients.create(uri)) {
                ...
                doc.put("request from app", String.valueOf(request));
                doc.put("request to 3rd party API", cocktailURL);
                doc.put("response from 3rd party API", drinks);
                System.out.println("start inserting to mongo db");
                collection.insertOne(doc);
                System.out.println("finished insertion");
            } catch (Exception e){
                System.out.println("unable to insert to mongo db");
            }
                nextView = "result.jsp";
        } else {
            ...
            System.out.println("display index jsp");
            nextView = "index.jsp";
        }
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println("servlet failed here when transferring control.");
//            throw new RuntimeException(e);
        } catch (IOException e) {
            System.out.println("Unsupported Encoding Exception here.");
//            throw new RuntimeException(e);
